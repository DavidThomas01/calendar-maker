...
interface GeneralDayCommentsProps {
  ...
  comments: DayComment[];
  isOwner: boolean;
  onCommentsUpdate: (dayBookingId: string, comments: DayComment[]) => void;
}
...
// Generate a special booking ID for day comments
...
const GeneralDayComments: React.FC<GeneralDayCommentsProps> = ({
  date,
  apartmentName,
  comments,
  isOwner,
  ...
  const dayBookingId = generateDayBookingId(date, apartmentName);
      ...
      {comments.map((comment) => (
        <div key={comment.id} className="group">
          {editingCommentId === comment.id ? (
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded p-2">
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-1">
                  <Type className="h-3 w-3 text-yellow-600" />
                  <select
                    value={editFontSize}
            ...
            // Display Mode - Clean comment display with yellow accent for day comments
            <div className="relative">
              <div className={`${getFontSizeClass(comment.fontSize)} text-gray-700 leading-tight p-1 rounded bg-yellow-50 border-l-2 border-yellow-400`}>
                {comment.text}
              </div>
              {isOwner && (
        ...
        <div>
          {isAddingComment ? (
            <div className="bg-yellow-50 border border-yellow-200 rounded p-2">
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={2}
                  disabled={isLoading}
                  placeholder="Escribe tu comentario para este día..."
                  autoFocus
                />
                <div className="flex gap-1">
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 bg-yellow-600 text-white text-xs rounded hover:bg-yellow-700 disabled:opacity-50"
                  >
                    <Save className="h-3 w-3" />
                    Guardar
                  </button>
            ...
            <button
              ...
              className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-1 px-2 py-1 bg-yellow-100 text-yellow-700 text-xs rounded hover:bg-yellow-200 no-print"
            >
...
export default GeneralDayComments;